...
const navItems = [
  ...
  { id: 'customers',   label: 'Customers',    path: '/technician/customers',   d: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2a3 3 0 00-6 0v2m6 0H7m6-6a4 4 0 11-8 0 4 4 0 018 0zm6 0a4 4 0 11-8 0 4 4 0 018 0z' },
]
...
export default function TechnicianPerformancePage() {
  ...
  const tickRef   = useRef(null)          // ← ADD
const [countdown, setCountdown] = useState(30)  // ← ADD
...
const resetTimer = useCallback(() => {
  ...
  tickRef.current = setInterval(() => {
    setCountdown(c => {
      if (c <= 1) {
        fetchPerformance(period, customStart, customEnd)
        return 30
      }
      return c - 1
    })
  ...
  fetchPerformance(period).then(() => resetTimer())
  return () => {
  ...
  setPeriod(p)
  if (p !== 'custom') fetchPerformance(p).then(() => resetTimer())
}

  const handleCustomApply = () => {
  fetchPerformance('custom', customStart, customEnd).then(() => resetTimer())
}
  ...
  <span style={{fontSize:12, color:'var(--pale)', whiteSpace:'nowrap'}}>
    ↻ in {countdown}s
  </span>
  <button
    className={`tp-refresh-btn${spinning ? ' spinning' : ''}`}
    onClick={() => fetchPerformance(period, customStart, customEnd).then(() => resetTimer())}
  >
